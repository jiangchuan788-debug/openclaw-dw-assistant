...
# -*- coding: utf-8 -*-
"""
优化的告警查询脚本 - 使用 sessions_send 方案（已验证可靠）
- 时间：昨天到今天
- 过滤：status=0 + 未恢复的所有告警（不区分层级）
- 格式：保持原有告警格式
- 发送：使用 sessions_send 直接发送到钉钉群（不依赖Webhook）
...
def main():
    ...
    for i, alert in enumerate(alerts, 1):
        ...
        print(f"[{i}/{len(alerts)}] 发送告警 ID:{alert['id']}...")
    ...
    # 发送总结
    summary = f"📊 告警发送完成\n共计 {len(alerts)} 条告警，已处理 {len(processed_ids)} 条"
